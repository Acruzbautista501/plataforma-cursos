...
class CursoController {
    ...
    public function obtenerProgreso($usuario_id, $curso_id) {
        if (empty($usuario_id) || empty($curso_id)) {
            return ['error' => 'El ID del usuario y del curso son obligatorios.'];
        }
        $progreso = $this->curso->obtenerProgresoCurso($usuario_id, $curso_id);
        return ['progreso' => $progreso];
    }

    // Obtener usuarios que han terminado un curso
    public function obtenerUsuariosCompletaron($curso_id) {
        if (empty($curso_id)) {
            return ['error' => 'El ID del curso es obligatorio.'];
        }
        return $this->curso->obtenerUsuariosCompletaronCurso($curso_id);
    }

    // Obtener los cursos con más usuarios
    public function obtenerCursosConMasUsuarios() {
        return $this->curso->obtenerCursosConMasUsuarios();
    }
